...
int down_tweak(int argc, char **argv)
{
	if (argv[1])
		if (argv[2])
			downTweakWeb(argv[1], argv[2]);
		else
			printf(RED "Expected a filename, " GRN "Example: " CYN "pie tweak download https://example-pie-tweak/tweak.c tweak.c");
	else
		printf(RED "Expected a valid url and a filename, " GRN "Example: " CYN "pie tweak download https://example-pie-tweak/tweak.c tweak.c");
